...
const VinSearch = () => {
  ...
  const handleSearch = async (e) => {
    ...
    try {
      // Use v2 API with Bitmotors enrichment
      const response = await axios.get(`${API_URL}/api/v2/search/${vin.toUpperCase()}`);
      ...
      if (response.data && response.data.success) {
        ...
        setResult({
          success: true,
          vin: v2Data.vin,
          message: `Знайдено через ${v2Data.winning_source || 'bitmotors'}`,
          source: v2Data.winning_source || 'bitmotors',
          searchDurationMs: v2Data.response_time_ms || 0,
          cached: v2Data.cached,
          vehicle: {
            vin: v2Data.vin,
            make: v2Data.make,
            model: v2Data.model,
            vehicleModel: v2Data.model,
            year: v2Data.year,
            title: `${v2Data.year || ''} ${v2Data.make || ''} ${v2Data.model || ''}`.trim(),
            images: v2Data.image_urls || [],
            primaryImage: v2Data.image_urls?.[0],
            odometer: v2Data.odometer,
            odometerUnit: v2Data.odometer_unit,
            location: v2Data.location,
            saleDate: v2Data.sale_date,
            lotNumber: v2Data.lot_number,
            auctionName: v2Data.auction_name,
            damageType: v2Data.damage_primary,
            condition: v2Data.condition,
            transmission: v2Data.transmission,
            fuelType: v2Data.fuel_type,
            engine: v2Data.engine,
            drivetrain: v2Data.drivetrain,
            keys: v2Data.keys,
            seller: v2Data.seller,
            titleStatus: v2Data.title,
            sourceUrl: v2Data.source_url,
            price: v2Data.price,
          }
        });
      } else {
        setError(response.data?.message || 'VIN не знайдено');
      }
      
      // Refresh sources to see updated stats
      fetchSources();
    } catch (err) {
      setError(err.response?.data?.message || err.response?.data?.error || 'Помилка пошуку');
    } finally {
      setLoading(false);
    }
  };
