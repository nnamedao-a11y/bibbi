...
export default function VinCheckPage() {
  ...
  useEffect(() => {
    ...
    axios.get(`${API}/api/vin/search/${encodeURIComponent(vin)}`)
      ...
      .catch((e) => setError(e.response?.data?.detail || 'VIN lookup failed'))
    ...
    <div data-testid="vin-check-page" className="bg-black min-h-[60vh]">
      <section className="pt-12 pb-20">
        <div className="max-w-[1920px] mx-auto px-6 lg:px-[100px]">
          <Breadcrumbs items={[{ label: 'HOME', to: '/' }, { label: 'VIN CHECK' }]} />
          <h1 className="text-[48px] md:text-[80px] font-bold uppercase text-[#FEAE00] mt-10 leading-none">VIN check</h1>
          ...
          <form onSubmit={submit} className="mt-12 flex items-center gap-3 max-w-[640px]">
            <div className="flex-1 flex items-center gap-2 h-12 px-4 border border-[#555452] rounded-lg">
              ...
              <input value={q} onChange={(e) => setQ(e.target.value)} placeholder="Enter VIN or LOT number" className="flex-1 bg-transparent outline-none text-[14px] uppercase text-white placeholder:text-[#5E5E5E]" data-testid="vin-check-input" />
            </div>
            <button className="btn-amber h-12" data-testid="vin-check-submit">Check</button>
          </form>

          {loading && <div className="mt-12 text-[#5E5E5E]">Looking up VIN…</div>}
          {error && <div className="mt-12 text-red-400">{error}</div>}

          {data && (
            <div className="mt-12 bg-[#1D1D1B] rounded-lg p-8">
              <h2 className="text-[24px] font-bold text-white mb-6">Result</h2>
